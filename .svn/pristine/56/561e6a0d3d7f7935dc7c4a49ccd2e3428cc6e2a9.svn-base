...
<div id="content" class="content content-full-width">
    ...
    <div class="vertical-box table">
      <ol class="bwizard-steps clearfix clickable" >
        ...
        $tp=App\Models\Project_Task::where('project_id','=',$data->id)->where('task_type','=',$ts->task_level)->where('status','=',null)->count();
         ?>
          <li role="tab" aria-selected="false" @if($ts->num==0 && $ts->status==1)style="background: #00acac!important;"@endif ><span class="label">{{$ts->task_type}}</span><a href="/task/show/{{$ts->project_id}}_{{$ts->task_type}}" class="hidden-phone">
    ...
    <div class="vertical-box">
                ...
                <div class="wrapper bg-silver-lighter">
                    ...
                    <div class="btn-toolbar">
                      <div class="btn-group">
                        ...
                        <a data-toggle="modal" href="#modal-product-edit_product" class="btn btn-success p-l-40 p-r-40 btn-sm" @if(!strstr($data->exec_user,Auth::user()->name)) disabled="disabled"  @endif >
                            更改状态
                        </a>
                        <a data-toggle="modal" href="#modal-product-log" class="btn btn-success p-l-40 p-r-40 btn-sm ">
                            log
                        </a>
        ...
        <form class="form-horizontal" method="POST" action="/project_task_attr_edit" onsubmit="return false" id="form1">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                      <label class="col-md-12  form-control" >
                                        @if(App\Models\Project_Task_attr::where('project_id','=',$data->id)->where('status','=',0)->orderby('task_type')->first())
                                        {{App\Models\Task_attr::where('project_level','=',$data->project_type)->where('task_level','=',App\Models\Project_Task_attr::where('project_id','=',$data->id)->where('status','=',0)->orderby('task_type')->first()->task_type)->first()->task_type}}
                                        </label>
                                        @endif
                                </div>
                            </div>
            ...
            <div class="modal fade" id="modal-product-log">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                            <h4 class="modal-title">log-日志</h4>
                        </div>
                        <div class="modal-body" style="height:400px; overflow:auto">
                          @foreach($log as $l)
                          {{$l->edit_user}}--{{$l->task_attr}}-----------{{$l->created_at}}<br><hr>
                          @endforeach
                        </div>
                        <div class="modal-footer">
                            <a href="javascript:;"  class="btn btn-sm btn-white" data-dismiss="modal">取消</a>
